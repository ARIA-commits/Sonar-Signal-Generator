    ...
    try:
        ...
        for abr in selected_types:
            for i in range(n_samples):
                ...
                # 生成CSV数据（使用utf-8-sig编码确保中文正常显示）
                df = pd.DataFrame({"时间(s)": t_vals, "信号幅度": signal, "磨蚀类别": abr_type})
                csv_buffer = StringIO()
                # 关键修改：确保DataFrame导出时使用正确编码
                df.to_csv(csv_buffer, index=False, encoding='utf-8-sig')
                csv_data = csv_buffer.getvalue().encode('utf-8-sig')  # 二次确认编码
        ...
        if summary_data:
            ...
            summary_data = summary_buffer.getvalue().encode('utf-8-sig')
...
# Streamlit界面（优化单个文件下载的编码）
def gui():
    st.set_page_config(page_title="声纳信号生成器", layout="wide")
    st.title("智能声纳信号生成器")
    ...
    with st.container(border=True):
        ...
        if generate_btn:
            ...
            csv_buffer = StringIO()
            df.to_csv(csv_buffer, index=False, encoding='utf-8-sig')
            csv_data = csv_buffer.getvalue().encode('utf-8-sig')  # 确保编码正确
            st.download_button(
    ...
    with st.container(border=True):
        ...
        sample_num = st.number_input("每类样本数", min_value=1, max_value=20, value=3, step=1)
